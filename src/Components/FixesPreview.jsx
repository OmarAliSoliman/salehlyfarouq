...
class FixesPreview extends Component {
  state = {
    data: [],
    names: [],
    allDates: [],
  ...
  async componentDidMount() {
    ...
    await axios
      .get(
        `http://sal7lly-001-site1.ctempurl.com/api/Orders/GetAllOrdersByStep?OrderSteps=50f35956-71cb-4e5f-4288-08d9a6f4d6e9${date == "" ? "" : `&date=${date}`}`
      )
      .then((res) => {
        // console.log(res);
        if (res.status === 200) {
      ...
      axios
      .get(
        "http://sal7lly-001-site1.ctempurl.com/api/Orders/GetAllOrdersByStep?OrderSteps=50f35956-71cb-4e5f-4288-08d9a6f4d6e9"
      )
      .then((res) => {
  ...
  render() {
    ...
    return (
      <div>
        <h5 class="mb-3 text-center font-weight-bold h4">معاينة</h5>
